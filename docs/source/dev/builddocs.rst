...
`pavics-sdi`_ is also being translated to French, and it is possible to add other languages. For example to add a German translation, run ``sphinx-intl`` from the :file:`docs/` directory with the ``de`` locale:
